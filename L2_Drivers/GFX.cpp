...
*/
/**************************************************************************/
void GFX::writeFastVLine(int16_t x, int16_t y,
...
*/
/**************************************************************************/
void GFX::writeFastHLine(int16_t x, int16_t y,
  ...
  const uint8_t bitmap[], int16_t w, int16_t h, uint16_t color) {
    ...
    for(int16_t j=0; j<h; j++, y++) {
        for(int16_t i=0; i<w; i++) {
            if(i & 7) byte <<= 1;
            else      byte   = (bitmap[j * byteWidth + i / 8]);//pgm_read_byte(&bitmap[j * byteWidth + i / 8]);
...
void GFX::drawGrayscaleBitmap(int16_t x, int16_t y,
  const uint8_t bitmap[], int16_t w, int16_t h) {
    ...
    for(int16_t j=0; j<h; j++, y++) {
        for(int16_t i=0; i<w; i++ ) {
            writePixel(x+i, y, (bitmap[j * w + i]));//pgm_read_byte(&bitmap[j * w + i]));
        }
    }
    endWrite();
}
...
void GFX::drawGrayscaleBitmap(int16_t x, int16_t y,
  ...
  int16_t w, int16_t h) {
    ...
    for(int16_t j=0; j<h; j++, y++) {
        for(int16_t i=0; i<w; i++ ) {
            if(i & 7) byte <<= 1;
            else      byte   = (mask[j * bw + i / 8]);//pgm_read_byte(&mask[j * bw + i / 8]);
            if(byte & 0x80) {
                writePixel(x+i, y, (bitmap[j * w + i]));//pgm_read_byte(&bitmap[j * w + i]));
            }
        }
...
*/
/**************************************************************************/
void GFX::drawRGBBitmap(int16_t x, int16_t y,
  const uint16_t bitmap[], int16_t w, int16_t h) {
    startWrite();
    for(int16_t j=0; j<h; j++, y++) {
        for(int16_t i=0; i<w; i++ ) {
            writePixel(x+i, y, (bitmap[j * w + i]));//pgm_read_word(&bitmap[j * w + i]));
        }
    }
    endWrite();
}
